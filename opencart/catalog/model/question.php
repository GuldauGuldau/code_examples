...
class ModelYmreportQuestion extends Model {
	...
	public function getQuestions() {
		$query = $this->db->query("SELECT DISTINCT *,
			(SELECT COUNT(yqr.rating) FROM " . DB_PREFIX . "ym_question_rating yqr
			WHERE yqr.rating = '1' AND yqr.id_question = yq.id) AS qslike,
			(SELECT COUNT(yqr.rating) FROM " . DB_PREFIX . "ym_question_rating yqr WHERE yqr.rating = '0' AND yqr.id_question = yq.id) AS qsdislike,
			yq.id as qsid FROM " . DB_PREFIX . "ym_question yq LEFT JOIN " . DB_PREFIX . "ym_question_rating yqr ON(yq.id = yqr.id_question) GROUP BY yq.id ORDER BY (qslike - qsdislike) DESC");
		return $query->rows;
	}
